...
DO $$
BEGIN
    BEGIN ALTER TABLE businesses ADD COLUMN google_place_id VARCHAR UNIQUE;
    EXCEPTION WHEN duplicate_column THEN NULL; END;
    
    BEGIN ALTER TABLE businesses ADD COLUMN google_rating DECIMAL(3,2);
    EXCEPTION WHEN duplicate_column THEN NULL; END;
    
    BEGIN ALTER TABLE businesses ADD COLUMN google_review_count INTEGER DEFAULT 0;
    EXCEPTION WHEN duplicate_column THEN NULL; END;
    
    BEGIN ALTER TABLE businesses ADD COLUMN partnership_tier VARCHAR(20) DEFAULT 'basic';
    EXCEPTION WHEN duplicate_column THEN NULL; END;
    
    BEGIN ALTER TABLE businesses ADD COLUMN is_featured BOOLEAN DEFAULT false;
    EXCEPTION WHEN duplicate_column THEN NULL; END;
    
    BEGIN ALTER TABLE businesses ADD COLUMN display_priority INTEGER DEFAULT 0;
    EXCEPTION WHEN duplicate_column THEN NULL; END;
    
    BEGIN ALTER TABLE businesses ADD COLUMN quality_score INTEGER DEFAULT 0;
    EXCEPTION WHEN duplicate_column THEN NULL; END;
    
    BEGIN ALTER TABLE businesses ADD COLUMN source VARCHAR(50) DEFAULT 'manual';
    EXCEPTION WHEN duplicate_column THEN NULL; END;
    
    BEGIN ALTER TABLE businesses ADD COLUMN curation_notes TEXT;
    EXCEPTION WHEN duplicate_column THEN NULL; END;
END $$;
...
BEGIN
    IF rating IS NOT NULL THEN score := score + (rating * 8)::INTEGER; END IF;
    IF review_count IS NOT NULL THEN score := score + LEAST(review_count / 4, 25)::INTEGER; END IF;
...
CREATE TRIGGER update_suggested_businesses_updated_at BEFORE UPDATE ON suggested_businesses FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_discovery_campaigns_updated_at BEFORE UPDATE ON discovery_campaigns FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Insert sample discovery campaigns
INSERT INTO discovery_campaigns (name, description, target_location, center_latitude, center_longitude, search_radius, target_categories, quality_filters, run_frequency) VALUES 
('Hua Hin Premium Restaurants', 'High-quality dining establishments in Hua Hin center', 'Hua Hin, Thailand', 12.5664, 99.9589, 5000, ARRAY['Restaurants'], '{"minRating": 4.0, "minReviewCount": 20}', 'weekly'),
('Bangkok Thonglor Wellness', 'Spas and wellness centers in premium Bangkok district', 'Bangkok Thonglor', 13.7326, 100.5848, 3000, ARRAY['Wellness'], '{"minRating": 4.2, "minReviewCount": 15}', 'monthly'),
('Pattaya Entertainment', 'Entertainment venues and activities in Pattaya', 'Pattaya, Thailand', 12.9236, 100.8825, 8000, ARRAY['Entertainment'], '{"minRating": 3.8, "minReviewCount": 30}', 'weekly')
ON CONFLICT (name) DO NOTHING;

-- Insert sample suggested businesses
INSERT INTO suggested_businesses (google_place_id, name, address, latitude, longitude, phone, website_url, google_rating, google_review_count, primary_category, quality_score, curation_status, discovery_source) VALUES 
('ChIJ_test_restaurant_1', 'The Deck Restaurant & Bar', '123 Beach Road, Hua Hin, Thailand', 12.5664, 99.9589, '+66 32 123 456', 'https://thedeckhuahin.com', 4.4, 87, 'Restaurants', 82, 'pending', 'automated_campaign'),
('ChIJ_test_spa_1', 'Ananda Wellness Spa', '456 Wellness Lane, Bangkok, Thailand', 13.7326, 100.5848, '+66 2 456 789', 'https://anandawellness.com', 4.6, 124, 'Wellness', 89, 'pending', 'manual_search')
ON CONFLICT (google_place_id) DO NOTHING;

-- VERIFICATION QUERIES
SELECT 'Tables Created' as status, table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('suggested_businesses', 'sales_leads', 'discovery_campaigns', 'curation_activities', 'business_metrics');
