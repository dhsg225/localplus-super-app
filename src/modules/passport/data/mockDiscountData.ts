...
export const mockDiscountBusinesses: DiscountBusiness[] = [
  // Restaurants & Cafés
  {
    id: 'rest-001',
...
export const getBusinessesByCategory = (category: string) => {
  return mockDiscountBusinesses.filter(business => business.category === category);
};

export const getPopularBusinesses = () => {
  return mockDiscountBusinesses.filter(business => business.isPopular);
};

export const getNewBusinesses = () => {
  return mockDiscountBusinesses.filter(business => business.isNew);
};

export const getCategoryStats = () => {
  const categories = mockDiscountBusinesses.reduce((acc, business) => {
    ...
    return acc;
  }, {} as Record<string, { count: number; avgDiscount: number }>);
