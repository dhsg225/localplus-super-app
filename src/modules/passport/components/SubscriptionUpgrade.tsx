...
var SubscriptionUpgrade = function () {
    var navigate = useNavigate();
    var _a = useState('premium-monthly'), selectedTier = _a[0], setSelectedTier = _a[1];
    var _b = useState(false), isProcessing = _b[0], setIsProcessing = _b[1];
    var freeTier = mockSubscriptionTiers.find(function (t) { return t.id === 'free'; });
    var monthlyTier = mockSubscriptionTiers.find(function (t) { return t.id === 'premium-monthly'; });
    var yearlyTier = mockSubscriptionTiers.find(function (t) { return t.id === 'premium-yearly'; });
    var handleUpgrade = function () { return __awaiter(void 0, void 0, void 0, function () {
    ...
    var CompareFeature = function (_a) {
        var feature = _a.feature, free = _a.free, premium = _a.premium;
        return (<div className="flex items-center justify-between py-3 border-b border-gray-100">
      <span className="text-sm text-gray-700">{feature}</span>
      <div className="flex items-center space-x-8">
        <div className="w-16 text-center">
          {free ? (<Check size={16} className="text-green-500 mx-auto"/>) : (<span className="text-gray-300 text-xs">—</span>)}
        </div>
        <div className="w-16 text-center">
          {premium ? (<Check size={16} className="text-green-500 mx-auto"/>) : (<span className="text-gray-300 text-xs">—</span>)}
        </div>
      </div>
    </div>);
    };
    return (<div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="px-4 py-4">
          <div className="flex items-center space-x-3">
            <button onClick={function () { return navigate('/passport'); }} className="p-2 hover:bg-gray-100 rounded-lg">
      ...
      <div className="p-4">
        ...
        <div className="space-y-4 mb-6">
          {/* Monthly Plan */}
          <div onClick={function () { return setSelectedTier('premium-monthly'); }} className={"bg-white rounded-lg border-2 p-4 cursor-pointer transition-all ".concat(selectedTier === 'premium-monthly'
            ? 'border-orange-500 bg-orange-50'
            : 'border-gray-200 hover:border-gray-300')}>
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className={"w-5 h-5 rounded-full border-2 ".concat(selectedTier === 'premium-monthly'
            ? 'border-orange-500 bg-orange-500'
            : 'border-gray-300')}>
                  {selectedTier === 'premium-monthly' && (<div className="w-full h-full rounded-full bg-white scale-50"/>)}
                </div>
                <div>
          ...
          {/* Yearly Plan */}
          <div onClick={function () { return setSelectedTier('premium-yearly'); }} className={"bg-white rounded-lg border-2 p-4 cursor-pointer transition-all ".concat(selectedTier === 'premium-yearly'
            ? 'border-green-500 bg-green-50'
            : 'border-gray-200 hover:border-gray-300')}>
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className={"w-5 h-5 rounded-full border-2 ".concat(selectedTier === 'premium-yearly'
            ? 'border-green-500 bg-green-500'
            : 'border-gray-300')}>
                  {selectedTier === 'premium-yearly' && (<div className="w-full h-full rounded-full bg-white scale-50"/>)}
                </div>
                <div>
        ...
        {/* Upgrade Button */}
        <button onClick={handleUpgrade} disabled={isProcessing} className="w-full bg-gradient-to-r from-orange-500 to-red-500 text-white py-4 rounded-lg text-lg font-semibold disabled:opacity-50">
          {isProcessing ? (<div className="flex items-center justify-center space-x-2">
              <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"/>
              <span>Processing...</span>
            </div>) : ("Upgrade to Premium - \u0E3F".concat(selectedTier === 'premium-yearly' ? yearlyTier.price : monthlyTier.price))}
        </button>
