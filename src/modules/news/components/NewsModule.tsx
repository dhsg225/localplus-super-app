...
const NewsModule: React.FC<NewsModuleProps> = ({ className = '' }) => {
    ...
    const detectLocation = () => {
      try {
        ...
        if (savedLocation) {
          ...
          const detectedCity = cityMapping[locationData.city as keyof typeof cityMapping] || 'hua-hin';
          
          // Only set if we have news for this city
          if (availableCities[detectedCity as keyof typeof availableCities]) {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const popularCategories = data.filter((cat: NewsCategory) => cat.count > 0).slice(0, 10);
