...
const generateRandomDeals = (restaurantId: string, restaurantName: string, restaurantImage: string, cuisine: string, location: string, rating: number, reviewCount: number, basePrice: number) => {
  const dealTypes = ['early-bird', 'afternoon', 'late-night'];
  ...
  dealTypes.forEach((dealType, typeIndex) => {
    ...
    deals.push({
      id: `off-peak-${restaurantId}-${dealCounter}`,
      restaurantId,
      restaurantName,
      restaurantImage,
      cuisine,
      ...
      discountPercentage,
      dealType: dealType as 'early-bird' | 'afternoon' | 'late-night',
      description: dealDescriptions[dealType as keyof typeof dealDescriptions],
      terms: [
        `${discountPercentage}% discount special`,
        'Limited time offer',
      ...
      availableDates,
      timeSlots: timeSlots[dealType as keyof typeof timeSlots],
...
// Generate all deals
export const mockOffPeakDeals: OffPeakDeal[] = restaurants.flatMap(restaurant => 
  generateRandomDeals(
    restaurant.id,
    restaurant.name,
    restaurant.image,
