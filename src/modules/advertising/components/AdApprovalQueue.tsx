...
import { CheckCircle, XCircle, Eye, Clock, DollarSign, MapPin, Calendar, AlertTriangle } from 'lucide-react';
var AdApprovalQueue = function () {
    var _a = useState('all'), filter = _a[0], setFilter = _a[1];
    var _b = useState(null), selectedAd = _b[0], setSelectedAd = _b[1];
    ...
    ]), pendingAds = _c[0], setPendingAds = _c[1];
    var filteredAds = pendingAds.filter(function (ad) {
        if (filter === 'all')
            return true;
        if (filter === 'pending')
            return ad.paymentStatus === 'pending';
        if (filter === 'paid')
            return ad.paymentStatus === 'paid';
        return true;
    });
    var handleApprove = function (adId) {
        setPendingAds(function (prev) { return prev.filter(function (ad) { return ad.id !== adId; }); });
        // In real app, this would call API to approve and activate the ad
        console.log("Approved ad: ".concat(adId));
        setSelectedAd(null);
    };
    var handleReject = function (adId, reason) {
        setPendingAds(function (prev) { return prev.filter(function (ad) { return ad.id !== adId; }); });
        // In real app, this would call API to reject and notify business
        console.log("Rejected ad: ".concat(adId, ", reason: ").concat(reason));
        setSelectedAd(null);
    };
    ...
    var formatTimeAgo = function (date) {
        var hours = Math.floor((Date.now() - date.getTime()) / (1000 * 60 * 60));
        if (hours < 1)
            return 'Just now';
        if (hours === 1)
            return '1 hour ago';
    ...
    return (<div className="max-w-7xl mx-auto p-6">
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            ...
            { key: 'paid', label: 'Paid', count: pendingAds.filter(function (ad) { return ad.paymentStatus === 'paid'; }).length },
            { key: 'pending', label: 'Payment Pending', count: pendingAds.filter(function (ad) { return ad.paymentStatus === 'pending'; }).length }
        ].map(function (tab) { return (<button key={tab.key} onClick={function () { return setFilter(tab.key); }} className={"px-3 py-1 text-sm font-medium rounded-md transition-colors ".concat(filter === tab.key
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    {pendingAds.filter(function (ad) { return ad.paymentStatus === 'paid'; }).length}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-600">
                    ฿{pendingAds.reduce(function (sum, ad) { return sum + ad.campaignBudget; }, 0).toLocaleString()}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-purple-600">
                    ฿{Math.round(pendingAds.reduce(function (sum, ad) { return sum + ad.campaignBudget; }, 0) / pendingAds.length || 0)}
                  </p>
                </div>
        ...
        <div className="divide-y divide-gray-200">
          {filteredAds.length === 0 ? (<div className="px-6 py-12 text-center">
            ...
            </div>) : (filteredAds.map(function (ad) { return (<div key={ad.id} className="px-6 py-4 hover:bg-gray-50">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      <h3 className="text-lg font-semibold text-gray-900">{ad.title}</h3>
                      <span className={"px-2 py-1 text-xs font-semibold rounded-full ".concat(getStatusColor(ad.paymentStatus))}>
                        {ad.paymentStatus === 'paid' ? '✓ Paid' : '⏳ Payment Pending'}
                      </span>
                      {ad.paymentStatus === 'pending' && (<AlertTriangle size={16} className="text-yellow-500"/>)}
                    </div>
                  ...
                  <div className="flex items-center space-x-2 ml-4">
                    <button onClick={function () { return setSelectedAd(ad); }} className="px-3 py-1 text-sm text-blue-600 border border-blue-600 rounded-lg hover:bg-blue-50">
                      <Eye size={14} className="inline mr-1"/>
                      Review
                    </button>
                    
                    {ad.paymentStatus === 'paid' && (<>
                        <button onClick={function () { return handleApprove(ad.id); }} className="px-3 py-1 text-sm text-white bg-green-600 rounded-lg hover:bg-green-700">
                          <CheckCircle size={14} className="inline mr-1"/>
                          Approve
                        </button>
                        <button onClick={function () { return handleReject(ad.id, 'Content policy violation'); }} className="px-3 py-1 text-sm text-white bg-red-600 rounded-lg hover:bg-red-700">
      ...
      {/* Review Modal */}
      {selectedAd && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-semibold text-gray-900">Review Advertisement</h3>
                <button onClick={function () { return setSelectedAd(null); }} className="text-gray-400 hover:text-gray-600">
                  <XCircle size={24}/>
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <span className={"inline-block px-2 py-1 text-xs font-semibold rounded-full ".concat(getStatusColor(selectedAd.paymentStatus))}>
              ...
              {selectedAd.submissionNotes && (<div className="mb-6">
              ...
              {/* Action Buttons */}
              {selectedAd.paymentStatus === 'paid' && (<div className="flex space-x-3">
                  <button onClick={function () { return handleApprove(selectedAd.id); }} className="flex-1 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 flex items-center justify-center">
                    <CheckCircle size={16} className="mr-2"/>
                    Approve & Go Live
                  </button>
                  <button onClick={function () { return handleReject(selectedAd.id, 'Requires modification'); }} className="flex-1 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 flex items-center justify-center">
                    <XCircle size={16} className="mr-2"/>
                    Reject
                  </button>
                </div>)}
              
              {selectedAd.paymentStatus === 'pending' && (<div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
