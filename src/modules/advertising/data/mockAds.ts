...
export const getAdsByPlacement = (placement: string): Advertisement[] => {
  return mockAdvertisements.filter(ad => 
    ad.isActive && ad.placement.includes(placement as any)
  ).sort((a, b) => b.priority - a.priority);
};

export const getInternalAds = (): Advertisement[] => {
  return mockAdvertisements.filter(ad => ad.type === 'internal' && ad.isActive);
};

export const getExternalAds = (): Advertisement[] => {
  return mockAdvertisements.filter(ad => ad.type === 'external' && ad.isActive);
};

export const getAdsByCategory = (category: string): Advertisement[] => {
  return mockAdvertisements.filter(ad => 
    ad.category === category && ad.isActive
  ).sort((a, b) => b.priority - a.priority);
}; 