...
import { MapPin, Star, DollarSign, Clock, Utensils, Coffee, Pizza, Fish, CheckCircle, XCircle, UserPlus, Eye, Phone, Navigation, Calendar, Menu } from 'lucide-react';
// Cuisine options with icons
export var CUISINE_FILTERS = [
...
// Price level options
export var PRICE_LEVEL_FILTERS = [
...
export var BUSINESS_TYPE_FILTERS = [
...
export var ACTION_CONFIGS = {
    // Consumer actions
    view: { label: 'View Details', icon: Eye, variant: 'primary' },
    call: { label: 'Call', icon: Phone, variant: 'outline' },
    directions: { label: 'Directions', icon: Navigation, variant: 'outline' },
    book: { label: 'Book Table', icon: Calendar, variant: 'primary' },
    menu: { label: 'View Menu', icon: Menu, variant: 'outline' },
    // Admin actions
    approve: { label: 'Approve', icon: CheckCircle, variant: 'success' },
    reject: { label: 'Reject', icon: XCircle, variant: 'danger' },
    lead: { label: 'Create Lead', icon: UserPlus, variant: 'outline' },
    details: { label: 'View Details', icon: Eye, variant: 'outline' },
    edit: { label: 'Edit', icon: Menu, variant: 'outline' },
};
// Default radius options (in meters)
export var DEFAULT_RADIUS = {
    consumer: 3000, // 3km for consumer searches
    admin: 5000, // 5km for admin discovery
};
// Maximum results per search
export var MAX_RESULTS = {
    consumer: 20,
    admin: 50,
};
// Map configuration
